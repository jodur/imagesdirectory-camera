...
def Getfileslist(path,exclude,begintime,endtime,extensions,lasthours=0.0):
	...
	files=[file for file in files if any(x in file for x in extensions ) and file not in exclude]
	
	#convert timestrings to epoch time    
	BeginTimeStamp=time.mktime(datetime.datetime.strptime(begintime, "%d/%m/%Y %H:%M:%S").timetuple())
	EndTimeStamp=time.mktime(datetime.datetime.strptime(endtime, "%d/%m/%Y %H:%M:%S").timetuple())

	#filter files between timestamps
	files=[file for file in files if GetTimestampFile(path,file)>=BeginTimeStamp and GetTimestampFile(path,file)<=EndTimeStamp ]
	...
	if lasthours>0.0 and len(files)>1:
		...
		files=[file for file in files if GetTimestampFile(path,file)>=(latest-(lasthours*3600)) and GetTimestampFile(path,file)<=EndTimeStamp ]
	return files
...
def setup(hass, config):
	...
	hass.services.register(
		DOMAIN, SERVICE_MOVE, Imagedirectory_Services,
