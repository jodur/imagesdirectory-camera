...
**Step 5**
A animated GIF named `latest_achtertuin.gif` is created in the given directory. This could be used, for example to notify your mobile phone with the animated GIF/MP4 as attachment. The event that will be generated by the service `imagedirectory.create_gif_mp4` after completion wil be used for this. See automation for creating a notification. In my case i only receive notifications when no one of my family is at home.

This file could also be used in a generic camera for displaying the latest event.

**Step 6**
In my case i use the camera entity, so i need to update the camera with the latest images from the occuring events. I could have use the timestamps that are also use for creating the animated GIF, but in my case i want a camera that show me the last 2 hours of occuring events
...
action:
  - service: notify.mobile_app_tf_jodur
    data:
      ...
      data:
        attachment:
          url: 'https://<your url to homeassistant>/local/latest_achtertuin.gif'
          content-type: gif
